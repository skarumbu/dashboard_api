...
**Date:** 2026-07-30  **Status:** Proposed  **PR:** [dashboard-api#13](https://github.com/skarumbu/dashboard_api/pull/13)

## Context
The dashboard API previously lacked visibility into open pull requests, limiting user insights into repository activity. This change integrates the GitHub Pulls API to fetch and display metadata for open PRs, enhancing transparency and usability.

## Decision
The API will fetch open pull requests from GitHub repositories and include them in the dashboard response. This improves user visibility into repository activity while ensuring reliability by gracefully handling API errors or missing tokens.
